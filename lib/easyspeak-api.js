...
// via lib/settings-store.js's getEasySpeakServer() (tmclub.eu by default —
// see EASYSPEAK_SERVERS there for the other two). Loaded into the
// background service worker via importScripts(). Unlike Basecamp, EasySpeak
// has no JSON API AND sits behind Cloudflare, which blocks programmatic
// fetch()/XHR requests outright (Cloudflare distinguishes a real page
// navigation from a fetch() via the Sec-Fetch-Mode/Sec-Fetch-Dest request
// headers, regardless of which extension context issues the fetch) — so
// unlike Basecamp, this cannot be done tab-lessly. Instead, this module
// navigates a real tab on the configured EasySpeak server (reusing one if
// already open, otherwise opening and focusing a new one so the user can
// solve an interactive Cloudflare challenge if one ever appears) and
// injects lib/easyspeak-parser.js into it via chrome.scripting to extract
// data from its live DOM.
//
...
async function scrapeAllEasySpeakClubs() {
  ...
  const { tabId, createdByUs } = await ensureEasySpeakTab(server);

  const { clubs } = await loadAndParse(
    tabId,
    `${root}/profile.php?mode=editprofile#tab_ti`,
    "parseProfileLinks"
  );
  ...
  for (const club of clubs) {
    const { members } = await loadAndParse(
      tabId,
      `${root}/memberchart.php?chart=10&c=${club.id}`,
      "parseMemberchart"
    );
...
/**
 * Finds an existing tab on the given EasySpeak server to reuse (focusing
 * it), or creates and focuses a new one if none is open.
 * @param {string} server e.g. "tmclub.eu" — see EASYSPEAK_SERVERS in
 *   lib/settings-store.js.
 * @returns {Promise<{tabId: number, createdByUs: boolean}>}
 */
async function ensureEasySpeakTab(server) {
  const [existing] = await chrome.tabs.query({ url: `https://${server}/*` });
  if (existing) {
...
function navigateAndWaitForRealPage(tabId, url, timeoutMs = PAGE_LOAD_TIMEOUT_MS) {
  ...
  return new Promise((resolve, reject) => {
    ...
    async function checkTab() {
      ...
      if (tab.url.startsWith(loginPath)) {
        if (!awaitingLogin) {
          awaitingLogin = true;
