...
// waitForBasecampLogin() below — and closed again once login completes.
...
const DASHBOARD_URL_PATTERN = `${DASHBOARD_ROOT}/*`;
...
/**
 * Finds an existing apps.basecamp.toastmasters.org tab to reuse (focusing
 * it), or creates and focuses a new blank one if none is open. Mirrors
 * ensureEasySpeakTab() in lib/easyspeak-api.js. Navigation is left to
 * waitForLoginRedirect(), so it doesn't matter whether a reused tab is
 * mid-navigation or already sitting on some other dashboard page.
 * @returns {Promise<{tabId: number, createdByUs: boolean}>}
 */
async function ensureBasecampDashboardTab() {
  const [existing] = await chrome.tabs.query({ url: DASHBOARD_URL_PATTERN });
  if (existing) {
...
/**
 * Opens (or reuses/focuses) a Basecamp dashboard tab, waits for the user to
 * land on the approvals page (logging in first if needed), then closes the
 * tab again — but only if this function created it itself, and only once
 * the wait actually succeeded. A pre-existing tab is never touched either
 * way; a self-created tab is left open on failure/timeout so the user can
 * see what went wrong. Mirrors scrapeAllEasySpeakClubs()'s cleanup rule.
 * @returns {Promise<void>}
 */
async function waitForBasecampLogin() {
  const { tabId, createdByUs } = await ensureBasecampDashboardTab();
