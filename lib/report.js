 ...
 * @param {{clubLookup?: object[], memberLinks?: object[], rejectedPairs?: object[], memberPathOverrides?: object[], pathAliasLookup?: Map<string,string>}} [resolution]
 ...
 *   persisted club ID pins — forced 1:1, bypassing CLUB_MATCH_THRESHOLD.
 ...
 * @returns {{basecamp: object|null, easyspeak: object|null, confidence: "confirmed"|"exact"|"fuzzy"|null, score: number|null}[]}
 */
function matchMembers(basecampPeople, easyspeakPeople, memberLinks = [], rejectedPairs = []) {
  ...
  for (const link of memberLinks) {
    if (bcById.has(link.basecampUserId) && esById.has(link.easyspeakMemberId)) {
      preAssigned.push({ aKey: link.basecampUserId, bKey: link.easyspeakMemberId, score: null, confidence: "confirmed" });
    }
  }
 ...
 */
function matchPaths(basecampPerson, easyspeakPerson, memberOverrides = [], pathAliasLookup = PATH_ALIAS_LOOKUP) {
 ...
 * @param {{memberLinks?: object[], rejectedPairs?: object[], memberPathOverrides?: object[], pathAliasLookup?: Map<string,string>}} [resolution]
...
function buildClubPairReport(basecampClub, easyspeakClub, resolution = {}, clubMatch = {}) {
  ...
  const memberMatches = matchMembers(basecampClub?.people ?? [], easyspeakClub?.people ?? [], memberLinks, rejectedPairs);
  ...
  const members = memberMatches.map(({ basecamp, easyspeak, confidence, score }) => {
    ...
    const { paths, easyspeakNoActivePath } = matchPaths(basecamp, easyspeak, overridesForMember, pathAliasLookup);
    const member = {
