...
//
// Storage I/O for the 5 persisted name-resolution keys in chrome.storage.local
// (memberLinks, memberRejectedPairs, clubLookup, pathLookup,
// memberPathOverrides). Unlike lib/report.js/lib/easyspeak-parser.js, this
// file is legitimately chrome.*-dependent (it's pure storage I/O) so it
// isn't Node/module.exports-able — same as lib/basecamp-api.js and
// lib/easyspeak-api.js. Loaded via a plain <script> tag (after lib/report.js,
// for PATH_ALIASES/buildPathAliasLookup) in members.html, settings.html, and
...
/**
 * Reads all 5 keys and shapes them into exactly what lib/report.js's
 * buildReport()/buildClubPairReport() 4th "resolution" param expects.
 * @returns {Promise<{memberLinks: object[], rejectedPairs: object[], clubLookup: object[], memberPathOverrides: object[], pathAliasLookup: Map<string,string>}>}
 */
