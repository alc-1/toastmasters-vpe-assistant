...
test(`globalSettings @ desktop`, async ({ page, pageUrl }) => {
  ...
  await page.waitForLoadState("networkidle");
  await page.waitForTimeout(300);
  await expect(page).toHaveScreenshot("globalSettings-desktop.png", { fullPage: true, animations: "disabled" });
});
...
test(`whatsNew @ desktop`, async ({ page, pageUrl }) => {
  ...
  await page.waitForLoadState("networkidle");
  await page.waitForTimeout(300);
  await expect(page).toHaveScreenshot("whatsNew-desktop.png", { fullPage: true, animations: "disabled" });
});
...
test(`popup @ default`, async ({ page, pageUrl }) => {
  ...
  await page.waitForLoadState("networkidle");
  await page.waitForTimeout(300);
  await expect(page).toHaveScreenshot("popup-default.png", { fullPage: true, animations: "disabled" });
});
