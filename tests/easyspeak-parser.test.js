...
const { parseProfileLinks, parseMemberchart, parseLevelCell } = require("../lib/easyspeak-parser.js");

const FIXTURE_DIR = path.join(__dirname, "..", "test-data", "easyspeak");
...
describe("parseMemberchart", () => {
  ...
  it("throws when the roster table can't be found", () => {
    const badDoc = new JSDOM('<html><body><table class="forumline"></table></body></html>').window.document;
...
describe("parseLevelCell", () => {
  function cellFrom(html) {
    ...
    return doc.getElementById("cell");
  }
  ...
  it("skips a bucket whose title doesn't match 'Complete N elective speech(es)'", () => {
    const td = cellFrom(
      '<span style="border:1px dashed #999;" title="Successful Club Series"><img src="images/icon_tick.gif"></span>'
    );
    expect(parseLevelCell(td)).toEqual({ needed: 0, done: 0 });
  });
